	...
	public static boolean load() {
		try {
			PreparedStatement sql = _DBMSGlobals.connection
					.prepareStatement("SELECT * FROM TRANS T, SALARY S WHERE T.NUMBER = S.TNUMBER");
			...
			while (rs.next()) {
				...
				Salary salary = new Salary(number, date, time, amount, type, description, bankAccountNo, chequeNo,
						entityName);
				salaries.add(salary);
				AppLogic.Transactions.add(salary);
			}
			AppLogic.Salaries.setList(salaries);
	...
	public static boolean save() {
		try {
			...
			for (int i = 0; i < salaries.size(); i++) {
				...
				sql.setInt(1, salaries.get(i).getNumber());
				sql.executeUpdate();
			}
			sql = _DBMSGlobals.connection.prepareStatement("MERGE SALARY S USING TEMP T ON S.TNUMBER = T.TNUMBER "
