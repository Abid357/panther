...
public class Rent extends JFrame {
	...
	private JComboBox<String> rentTypeCB;
	private JComboBox<String> licenseNoCB;

	public void setBackground(JLabel background) {
		BufferedImage img = null;
		try {
			img = ImageIO.read(new File(_GUIGlobals.directory + "\\Raw Materials\\Templates\\rent.jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public JComboBox<String> getRentTypeCB() {
		return rentTypeCB;
	}
	...
	public void setEditable(boolean isEnabled) {
		...
		getRentTypeCB().setEnabled(isEnabled);
	...
	private void initComponents() {
		...
		rentTypeCB = new JComboBox<String>();
		...
		rentTypeCB.setEditable(true);
		rentTypeCB.setBackground(_GUIGlobals.fontColor);
		rentTypeCB.setForeground(_GUIGlobals.fontColor);
		rentTypeCB.setFont(new Font("Century Gothic", 0, _GUIGlobals.getFontSize_18())); // NOI18N
		rentTypeCB.setModel(new DefaultComboBoxModel<>(new String[] { "" }));
		rentTypeCB.setBorder(null);
		rentTypeCB.setPreferredSize(new Dimension(getRentTypeCBSize().width, getRentTypeCBSize().height));
		getContentPane().add(rentTypeCB,
				new AbsoluteConstraints(getRentTypeCBPos().width, getRentTypeCBPos().height, -1, -1));
