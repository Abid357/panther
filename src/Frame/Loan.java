...
public class Loan extends JFrame {
	...
	private JComboBox<String> loanRefCB;
	...
	public void setBackground(JLabel background) {
		...
		try {
			img = ImageIO.read(new File(_GUIGlobals.directory + "\\Raw Materials\\Templates\\loan.jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public JComboBox<String> getLoanRefCB() {
		return loanRefCB;
	}
	...
	public Dimension getLoanRefCBSize() {
		double wfactor = 5.987525988;
		double hfactor = 36;
		return (new Dimension((int) (_GUIGlobals.screenWidth / wfactor), (int) (_GUIGlobals.screenHeight / hfactor)));
	}

	public Dimension getLoanRefCBPos() {
		double wfactor = 12.07391608;
		double hfactor = 1.877;
		return (new Dimension((int) (_GUIGlobals.screenWidth / wfactor), (int) (_GUIGlobals.screenHeight / hfactor)));
	}

	public Dimension getIsActiveCBSize() {
		double wfactor = 47.47252747;
		double hfactor = 26.78571429;
		return (new Dimension((int) (_GUIGlobals.screenWidth / wfactor), (int) (_GUIGlobals.screenHeight / hfactor)));
	}

	public Dimension getIsActiveCBPos() {
		double wfactor = 3.226288275;
		double hfactor = 1.893939394;
		return (new Dimension((int) (_GUIGlobals.screenWidth / wfactor), (int) (_GUIGlobals.screenHeight / hfactor)));
	}
	...
	public void setEditable(boolean isEnabled) {
		...
		getLoanRefCB().setEnabled(isEnabled);
		getIsActiveCB().setEnabled(isEnabled);
	}

	/**
	 * Creates new form Transaction
	 */
	public Loan(JPanel panel) {
		functionP = panel;
		initComponents();
	}
	...
	private void initComponents() {
		...
		loanRefCB = new JComboBox<String>();
		...
		typeS.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent arg0) {
				...
				if (slider.getValue() == 0)
					...
					loanType = "from";
				Functions.loadLoanCB(loanRefCB, (String) entityCB.getSelectedItem(), loanType, -1);
				isActiveCB.setEnabled(true);
				loanRefCB.setSelectedIndex(-1);
			}
		...
		loanRefCB.setBackground(_GUIGlobals.fontColor);
		loanRefCB.setForeground(_GUIGlobals.fontColor);
		loanRefCB.setFont(new Font("Century Gothic", 0, _GUIGlobals.getFontSize_15())); // NOI18N
		loanRefCB.setModel(new DefaultComboBoxModel<>(new String[] { "" }));
		loanRefCB.setBorder(null);
		loanRefCB.setPreferredSize(new Dimension(getLoanRefCBSize().width, getLoanRefCBSize().height));
		loanRefCB.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent evt) {
				...
				if (evt.getStateChange() == ItemEvent.SELECTED)
					...
					String selection = (String)((JComboBox<String>) evt.getSource()).getSelectedItem();
					if (selection != null)
						if (selection.equals(""))
							isActiveCB.setEnabled(true);
						else
		...
		getContentPane().add(loanRefCB,
				new AbsoluteConstraints(getLoanRefCBPos().width, getLoanRefCBPos().height, -1, -1));

		isActiveCB.setPreferredSize(new Dimension(getIsActiveCBSize().width, getIsActiveCBSize().height));
		isActiveCB.setForeground(_GUIGlobals.fontColor);
		isActiveCB.setBorder(null);
		isActiveCB.setSelected(true);
		getContentPane().add(isActiveCB,
				new AbsoluteConstraints(getIsActiveCBPos().width, getIsActiveCBPos().height, -1, -1));
