...
public class Transport extends JFrame {
	...
	private JCheckBox isTransportDetailsCB;
	private RectangularButton transportDetailsB;

	public void setBackground(JLabel background) {
		BufferedImage img = null;
		try {
			img = ImageIO.read(new File(_GUIGlobals.directory + "\\Raw Materials\\Templates\\transport\\Slide1.jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public JCheckBox getIsTransportDetailsCB() {
		return isTransportDetailsCB;
	}

	public RectangularButton getTransportDetailsB() {
		return transportDetailsB;
	}

	public Transport getFrame() {
		return this;
	}
	...
	private void initComponents() {
		...
		isTransportDetailsCB = new JCheckBox();
		transportDetailsB = new RectangularButton();
		transportTransactionNo = -1;
		items = null;
		...
		selectItemB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				if (isTransportDetailsCB.isSelected())
					Functions.create("ADD ITEM DETAILS");
				else
					new Notification("Please enter local transport details first.");
			}
		...
		isTransportDetailsCB
				.setPreferredSize(new Dimension(getIsTransportDetailsCBSize().width, getIsTransportDetailsCBSize().height));
		isTransportDetailsCB.setForeground(_GUIGlobals.fontColor);
		isTransportDetailsCB.setBorder(null);
		isTransportDetailsCB.setEnabled(false);
		isTransportDetailsCB.setSelected(false);
		getContentPane().add(isTransportDetailsCB,
				new AbsoluteConstraints(getIsTransportDetailsCBPos().width, getIsTransportDetailsCBPos().height, -1, -1));
		
		transportDetailsB.setPreferredSize(new Dimension(251, 45));
		transportDetailsB.setBorder(null);
		transportDetailsB.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				Functions.setTempObj(getFrame());
				Functions.create("ADD TRANSPORT DETAILS 1");
			}
		});
		getContentPane().add(transportDetailsB, new AbsoluteConstraints(341, 571, -1, -1));
